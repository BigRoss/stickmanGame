...
StickmanStateNormal::StickmanStateNormal(Stickman *stickman) : m_stickman(stickman), m_timer(1)
{

}

void StickmanStateNormal::changeSize(int x, int y){
    m_stickman->set_size("normal");
    m_stickman->construct(true, x);
    m_stickman->set_y(y);
    m_stickman->set_x(x);
}

bool StickmanStateNormal::collidesWithItem(QGraphicsItem *object){
...
bool StickmanStateNormal::collidesWithEnemy(QGraphicsPixmapItem *enemy){
    QGraphicsItem* head = m_stickman->get_head();
    QGraphicsItem* body = m_stickman->get_body();
    QGraphicsItem* left_arm = m_stickman->get_left_arm();
    QGraphicsItem* right_arm = m_stickman->get_right_arm();
...
//Normal velocity here
int StickmanStateNormal::set_movement_speed(int velocity){
    return velocity;
}

void StickmanStateNormal::update_power(){
    //No timer for the normal state as it is the default state
}

int StickmanStateNormal::get_timer(){
    return m_timer; //Normal state shouldn't time out
}

StickmanStateNormal::~StickmanStateNormal()
{
